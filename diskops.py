...
def read_data_from_file(project_name):
    try:
        return json.load(open(__get_project_file_name(project_name), "r"))
    except IOError:
        return {}
...
def write_data_to_file(project_name, latest_saved_data, new_version, new_ratings):
    ...
    version_rating_history = __get_rating_history_for_version(latest_saved_data, new_version)
    ...
    json.dump(latest_saved_data, open(__get_project_file_name(project_name), "w"))


def __get_rating_history_for_version(saved_data, new_app_version):
...
def __get_project_file_name(project_name):
